...
namespace he {
	...
	namespace {
		...
		void PlaySoundEffectAtIndex(SoundPlayer *player, Sound *sound, int index) {
			player->effects[index] = sound;
			Sound_Play(sound);
		}
	...
	void SoundPlayer_PlayEffect(SoundPlayer *player, const char *filename, const char *extn) {
		...
		for (int i = 0; i < MAX_EFFECTS; ++i) {
			if (player->effects[i] == 0) {
				PlaySoundEffectAtIndex(player, new Sound(filename, extn, 0), i);
				return;
			}
		}
...
he::SoundPlayer *g_SoundPlayer = nullptr;
